...
async function fetchStatusRows() {
  ...
  // Handle errors
  if (statusRes.error) console.error('Error fetching status view:', statusRes.error);
  if (parentRes.error) console.error('Error fetching parents count:', parentRes.error);
  if (camperRes.error) console.error('Error fetching campers count:', camperRes.error);

  // Process data rows - Exclude slot_label from the UI table if desired
  const rows: StatusRow[] = (statusRes.data || []).map((row) => {
    // We keep slot_label for logic but exclude it from the dynamic table display
            ...
            <thead>
              <tr className="bg-surface-container-lowest text-on-surface-variant text-sm border-b-2 border-surface-container">
                {Object.keys(rows[0]).map((key) => (
                  <th key={key} className="px-4 py-4 text-left font-bold uppercase tracking-wider">
                    {key.replaceAll('_', ' ')}
                  </th>
                ))}
              </tr>
              ...
              {rows.map((row: StatusRow, i: number) => (
                <tr 
                  ...
                  className={`transition-colors hover:bg-surface-container-lowest ${
                    i % 2 === 0 ? 'bg-white' : 'bg-surface-container-lowest/30'
                  }`}
                >
                  {Object.keys(row).map((k) => {
                    ...
                    // Detect and format ISO Timestamps
